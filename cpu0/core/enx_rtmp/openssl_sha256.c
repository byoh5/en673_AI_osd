...
# define UNALIGNED_P(p) ((unsigned int) (p) % alignof (uint32) != 0)
...
{
	printf("%s key len %d in len %d\n",__func__,keylen,inlen);
  ...
  printf("\n in %x %x %x %x\n",*((BYTE*)in+0), *((BYTE*)in+1), *((BYTE*)in+2), *((BYTE*)in+3));
...
int
hmac_sha256ex (const void *key, size_t keylen,
             const void *in, size_t inlen,const void *in2, size_t in2len, void *resbuf)
{printf("%s 1\n",__func__);
  struct sha256_ctx inner;
  struct sha256_ctx inner2;
  struct sha256_ctx outer;
  char optkeybuf[32]={0x00,};
  char block[64]={0x00,};
  char innerhash[32]={0x00,};
  char innerhash2[32]={0x00,};
  ...
  sha256_init_ctx (&inner);
  //sha256_init_ctx (&inner2);
  sha256_process_block (block, 64, &inner);
  printf("\n in %x %x %x %x inlen %d\n",*((BYTE*)in+0), *((BYTE*)in+1), *((BYTE*)in+2), *((BYTE*)in+3),inlen);
  sha256_process_bytes (in, inlen, &inner);
  printf("\n in2 %x %x %x %x in2len %d\n",*((BYTE*)in2+0), *((BYTE*)in2+1), *((BYTE*)in2+2), *((BYTE*)in2+3),in2len);
  sha256_process_bytes (in2, in2len, &inner);
  //sha256_process_block (block, 64, &inner2);
  //sha256_process_bytes (in2, in2len, &inner2);
  sha256_finish_ctx (&inner, innerhash);
  //sha256_finish_ctx (&inner2, innerhash2);
  printf("%s 5\n",__func__);
  ...
  sha256_process_block (block, 64, &outer);
  sha256_process_bytes (innerhash, 64, &outer);
  //sha256_process_bytes (innerhash2, 64, &outer);
