...
{//UartPutByte('1');
	...
	{
	//printf("v rtmp_list[%d].valid %d",i,rtmp_list[i].valid);
...
/////////////////////////////////////////////////////////////////////////////
		...
		prtmp_ses->ngx_s->in_buffer = (u_char*)malloc(sizeof(u_char)*(RTMP_PACKETSIZE*10));
		if(prtmp_ses->ngx_s->in_buffer == NULL)
		{
			printf("ERORR alloc inbuffer !!!!!!!!!!!!!!!!!\n");
			 return (rtmp_con_t*)NULL;
		}
		prtmp_ses->ngx_s->snd_buffer = (u_char*)malloc(sizeof(u_char)*RTMP_PACKETSIZE);
		...
		prtmp_ses->ngx_s->m_sndaudio = 0;
		prtmp_ses->ngx_s->m_sndvideo = 0;
			...
			case RTMP_HANDSHAKE_STATE:
				{
					...
					if(prtmp_ses->ngx_s->hs_stage == NGX_RTMP_HANDSHAKE_CLIENT_DONE)
					{
						...
						printf("ngx_rtmp_send_push_connect\r\n");
			...
			case RTMP_PUSH_STATE:
				{
...
#endif
					{
						...
						if(prtmp_ses->ngx_s->hs_stage == NGX_RTMP_PUSH_CLIENT_DONE)
						{
							...
							//prtmp_ses->ngx_s->lenofRTMP = 0;	
							prtmp_ses->ngx_s->playstart = 1;
							printf("session playstart RTMP_PUSH_LIVE\r\n");
...
#ifdef __AUDIO__
		//else if(s->m_sndvideo == 2 && s->m_sndaudio == 0)
		{
			s->m_sndaudio = 1;
			//printf("set sa 1\r\n");
