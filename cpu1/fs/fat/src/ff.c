...
#define	DEFINE_NAMEBUF		BYTE sfn[12+1]
...
#define	DEFINE_NAMEBUF		BYTE sfn[12+1]; WCHAR lbuf[_MAX_LFN + 1]
#define INIT_BUF(dobj)		{ (dobj).fn = sfn; (dobj).lfn = lbuf; }
#define	FREE_BUF()
#elif _USE_LFN == 3 		/* LFN feature with dynamic working buffer on the heap */
#define	DEFINE_NAMEBUF		BYTE sfn[12+1]; WCHAR *lfn
...
//#include <math.h>	// sdcard_fmtParmeter : ceilf

/*
	refer to C.2.4 Format Parameter Computations 
	in File System Specification Version 3.0. 

	@nom : Number of Sectors in Master Boot Record(NOM) -> default 8192
*/
static void sdcard_fmtParmeter(
	DWORD total_sectors, 			// TS
	DWORD nom, 						// NOM
	DWORD *p_sf, 					// SF(return)
	DWORD *p_rsc					// RSC(return)
)
{
#define Sectors_per_Cluuster	64	// SC
#define Sector_Size				512	// SS
#define FAT_bits				32	// FAT bits

	DWORD Secots_per_FAT;			// SF
	DWORD Reserved_Sector_Count;	// RSC
	DWORD n;

	/* Sectors per FAT is computed as following */
//	Secots_per_FAT = ceilf((total_sectors / Sectors_per_Cluuster * FAT_bits) / (Sector_Size * 8));
	Secots_per_FAT = ((total_sectors / Sectors_per_Cluuster * FAT_bits) / (Sector_Size * 8)) + 1;

	/* Reserved Sector Count is computed as following */
	/* Here, (n) means the minimum natural number satisfying above expression.*/
	for(n=1; n<10; n++)
	{
//		Reserved_Sector_Count = nom * n - 2 * Secots_per_FAT;
		if(nom * n > 2 * Secots_per_FAT)
			break;
	}
	Reserved_Sector_Count = nom * n - 2 * Secots_per_FAT;
//	printf("SF(%u), RSC(%u)\n", Secots_per_FAT, Reserved_Sector_Count);

	*p_sf = Secots_per_FAT;
	*p_rsc = Reserved_Sector_Count;
}


/* 
  SD CARD Format Parameter Recommendation
  - Sector per Cluster is 64
  - Sector Size is 512
  - FAT bits 32
  - Number of Sectors in Master boot Record is 
    NOM=BU(8192)
*/
/*-----------------------------------------------------------------------*/
...
#define N_FATS1		1		/* Number of FATs (1 or 2) */
#define N_FATS2		2
...
)
{
	...
	if (_MULTI_PARTITION && part) {
		...
		if (disk_ioctl(pdrv, GET_SECTOR_COUNT, &n_vol) != RES_OK || n_vol < 128)
			return FR_DISK_ERR;
		if(n_vol >= 7*(1024*1024*1024>>9)) { //if(n_vol > 8G equal or larger then sd card
			b_vol = (sfd) ? 0 : 8192;	/* Volume start sector, b_vol is relative Sector */
			au = 32 * 1024;
		} else {
	...
	if (fmt == FS_FAT32) {
		if(n_vol >= 7*(1024*1024*1024>>9)) { //if sd card is over 7GB
			sdcard_fmtParmeter(n_vol, b_vol, &n_fat, &n_rsv);
//			printf("Create Partition 8G over / TS(%u), NOM(%u), FS(%u), RSC(%u)\r\n", n_vol, b_vol, n_fat, n_rsv);
			au = 64;	// 64 <= 32KB
			n_dir = 0;
		} else {
//			printf("Create Partition 8G under\r\n");
...
//	printf("FAT area start sector(%u), b_vol(%u), n_rsv(%u)\r\n", b_fat, b_vol, n_rsv);
	if(pdrv == 0)
		b_dir = b_fat + n_fat * N_FATS1;		/* Directory area start sector */
	else
		b_dir = b_fat + n_fat * N_FATS2;
	...
	if(pdrv == 0) {
		...
		n_clst = (n_vol - n_rsv - n_fat * N_FATS1 - n_dir) / au;
	} else {
		n_clst = (n_vol - n_rsv - n_fat * N_FATS2 - n_dir) / au;
	}
	...
	if (_MULTI_PARTITION && part) {
		...
		if (sfd) {	/* No partition table (SFD) */
			...
			/** 
			Partition Table, here we only takes into account sd card more than 8GB
			Number of Heads = 255, Sectors per Track  = 63 
			*/
			if(pdrv == 0)
			{
			...
			{
//				printf("Partion table\r\n");
				tbl[1] = b_vol % (255 * 63) / 63;	/* off 1 starting Head  */
				ST_WORD(tbl + 2, ((b_vol/(255*63))<<6)|(b_vol%63+1));  /* starting sector / Starting Cylinder */      
//				printf("tbl[2~3] = 0x%X\r\n", ((b_vol/(255*63))<<6)|(b_vol%63+1));
				tbl[4] = 0xc;					/* system ID */
				tbl[5] = 0xfe;					/* Ending Head  */
#if 1
				ST_WORD(tbl + 6, 0xffff);  		/* Ending Sector, Ending Cylinder */
#else
				n = (b_vol + n_vol) / 63 / 255;
				tbl[6] = (BYTE)(n >> 2 | 63);	/* Partition end sector */
				tbl[7] = (BYTE)n;				/* End cylinder */
#endif
				ST_DWORD(tbl + 8, b_vol);		/* Relative Sector */
				ST_DWORD(tbl + 12, n_vol);		/* Total Sector*/
			}
	...
	mem_set(tbl, 0, SS(fs));
	mem_cpy(tbl, "\xEB\xFE\x90" "EYENIX__", 11);/* Boot jump code, OEM name */
	i = SS(fs);								/* Sector size */
...
//	printf("Reserved Sector Count : 0x%X\r\n", n_rsv);
	if(pdrv == 0) {
		sel_fats = N_FATS1;
		tbl[BPB_NumFATs] = N_FATS1;			/* Number of FATs */
	} else {
		sel_fats = N_FATS2;
		tbl[BPB_NumFATs] = N_FATS2;			/* Number of FATs */
	}
	...
	if (fmt == FS_FAT32) {
		...
		ST_DWORD(tbl + BPB_FATSz32, n_fat);	/* Number of sectors per FAT32 */
		ST_DWORD(tbl + BPB_RootClus, 2);	/* Root directory start cluster (2) */
		ST_WORD(tbl + BPB_FSInfo, 1);		/* FSINFO record offset (VBR + 1) */
		ST_WORD(tbl + BPB_BkBootSec, 6);	/* Backup boot record offset (VBR + 6) */
		tbl[BS_DrvNum32] = 0x80;			/* Drive number */
		tbl[BS_DrvNum32+1] = 0x0;			/* Reserved1 */
	...
	/* Initialize FAT area */
	wsect = b_fat;
//	printf("%u Sector zero start\r\n", wsect);
	for (i = 0; i < sel_fats; i++) {		/* Initialize each FAT copy */
...
//		printf("%u Sector tbl write\r\n", wsect);
		if (disk_write(pdrv, tbl, wsect++, 1) != RES_OK)
			return FR_DISK_ERR;
		mem_set(tbl, 0, SS(fs));			/* Fill following FAT entries with zero */
//		printf("%u Sector zero1 start\r\n", wsect);
		for (n = 1; n < n_fat; n++) {		/* This loop may take a time on FAT32 volume due to many single sector writes */
			if (disk_write(pdrv, tbl, wsect++, 1) != RES_OK)
				return FR_DISK_ERR;
		}
//		printf("%u Sector zero1 end\r\n", wsect-1);
	}

	/* Initialize root directory */
	i = (fmt == FS_FAT32) ? au : (UINT)n_dir;
//	printf("%u Sector zero2 start\r\n", wsect);
	do {
		if (disk_write(pdrv, tbl, wsect++, 1) != RES_OK)
			return FR_DISK_ERR;
	} while (--i);
//	printf("%u Sector zero2 end\r\n", wsect-1);
...
)
{
	...
	if(sz_disk <= 7*(1024*1024*1024>>9))
	{
		...
		/* We assume, this medium is sd card */
		mem_set(buf, 0, _MAX_SS);
		p = buf + MBR_Table; 
		p[1] = 8192 % (255 * 63) / 63;	/* off 1 starting Head  */
		ST_WORD(p + 2, ((8192/(255*63))<<6)|(8192%63+1));	/* starting sector / Starting Cylinder */
		p[4] = 0xC;								/* system ID */
		p[5] = 0xFE;							/* Ending Head  */
		ST_WORD(p + 6, 0xFFFF);					/* Ending Sector, Ending Cylinder */
		ST_DWORD(p + 8, 8192);					/* Relative Sector */
		sz_disk -= 8192;
		ST_DWORD(p + 12, sz_disk);				/* Total Sector*/
