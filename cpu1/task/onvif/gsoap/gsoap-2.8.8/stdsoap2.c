...
{ register int err;
  ...
  { const char *s = *soap_faultcode(soap);
    ...
    else if (soap->version == 2 && (!s || !strcmp(s, "env:Sender")))
      s = "400 Bad Request";
    else
...
{ if (soap_check_state(soap))
  ...
  { register char *q;
    ...
    soap_delete(soap, p); //This line was added !!!
...
soap_element(struct soap *soap, const char *tag, int id, const char *type)
{
  ...
  { if (soap->actor && *soap->actor)
    { if (soap_attribute(soap, soap->version == 2 ? "env:role" : "env:actor", soap->actor))
        return soap->error;
    }
    if (soap_attribute(soap, "env:mustUnderstand", soap->version == 2 ? "true" : "1"))
      return soap->error;
    soap->mustUnderstand = 0;
  }
  if (soap->encoding)
  { if (soap->encodingStyle && soap->local_namespaces)
    { if (!*soap->encodingStyle)
      ...
      if (soap->encodingStyle && soap_attribute(soap, "env:encodingStyle", soap->encodingStyle))
        return soap->error;
    }
    soap->encoding = 0;
  }
  ...
  { if (tp->visible && tp->value)
    {
      ...
      { if (!soap_match_tag(soap, tp->name, "SOAP-ENC:arrayType"))
        ...
        else if (!soap_match_tag(soap, tp->name, "env:actor"))
      ...
      else if (soap->version == 2)
      {
        ...
        if (!soap_match_tag(soap, tp->name, "SOAP-ENC:itemType"))
          ...
          strncpy(soap->arraySize, tp->value, sizeof(soap->arraySize) - 1);
              ...
              && (!strcmp(tp->value, "1") || !strcmp(tp->value, "true")))
          soap->mustUnderstand = 1;
        else if (!soap_match_tag(soap, tp->name, "env:role"))
        { if ((!soap->actor || strcmp(soap->actor, tp->value))
...
soap_envelope_begin_out(struct soap *soap)
{
  ...
  return soap_element_begin_out(soap, "env:Envelope", 0, NULL);
}
#endif
...
{ if (soap_element_end_out(soap, "env:Envelope")
   || soap_send_raw(soap, "\r\n", 2))	/* 2.8: always emit \r\n */
    return soap->error;
#ifndef WITH_LEANER
...
{ register struct Namespace *p;
  soap->part = SOAP_IN_ENVELOPE;
  if (soap_element_begin_in(soap, "env:Envelope", 0, NULL))
...
{ soap->part = SOAP_END_ENVELOPE;
  return soap_element_end_in(soap, "env:Envelope");
}
#endif
...
{ soap->part = SOAP_IN_BODY;
  ...
  if ((soap->mode & SOAP_SEC_WSUID) && soap_set_attr(soap, "wsu:Id", "Body", 1))
    return soap->error;
#endif
  return soap_element_begin_out(soap, "env:Body", 0, NULL);
}
#endif
...
{ if (soap_element_end_out(soap, "env:Body"))
    return soap->error;
...
{ soap->part = SOAP_IN_BODY;
  if (soap_element_begin_in(soap, "env:Body", 0, NULL))
    return soap->error;
  if (!soap->body)
...
{ if (soap->part == SOAP_NO_BODY)
    return soap->error = SOAP_OK;
  soap->part = SOAP_END_BODY;
  return soap_element_end_in(soap, "env:Body");
}
#endif
...
{ register int status = soap->error;
  ...
  { /* check flag set: check if SOAP Fault is present, if not just return */
    ...
    *soap_faultcode(soap) = (soap->version == 2 ? "env:Sender" : "env:Client");
    soap->error = status;
    soap_set_fault(soap);
  }
  else
  { register const char *s = *soap_faultcode(soap);
    if (!soap_match_tag(soap, s, "env:Server") || !soap_match_tag(soap, s, "env:Receiver"))
      status = SOAP_SVR_FAULT;
    else if (!soap_match_tag(soap, s, "env:Client") || !soap_match_tag(soap, s, "env:Sender"))
      status = SOAP_CLI_FAULT;
    else if (!soap_match_tag(soap, s, "env:MustUnderstand"))
      status = SOAP_MUSTUNDERSTAND;
    else if (!soap_match_tag(soap, s, "env:VersionMismatch"))
...
{ return soap_set_error(soap, soap->version == 2 ? "env:Sender" : "env:Client", NULL, faultstring, faultdetailXML, soaperror);
}
#endif
...
{ return soap_set_error(soap, soap->version == 2 ? "env:Receiver" : "env:Server", NULL, faultstring, faultdetailXML, soaperror);
}
#endif
...
{ return soap_copy_fault(soap, soap->version == 2 ? "env:Sender" : "env:Client", faultsubcodeQName, faultstring, faultdetailXML);
}
#endif
...
{ return soap_copy_fault(soap, soap->version == 2 ? "env:Receiver" : "env:Server", faultsubcodeQName, faultstring, faultdetailXML);
}
#endif
