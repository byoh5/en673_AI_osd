...
{ 
  ...
  if ((alg & (SOAP_SMD_PASSTHRU-1)) == SOAP_SMD_DGST_SHA1)
  { 
    data->ctx = (void*)SOAP_MALLOC(soap, sizeof(sha1_context));
  }
  else
  { 
    err = -1;
  ...
  { 
    case SOAP_SMD_DGST_SHA1:
      sha1_starts((sha1_context*)data->ctx);
      break;
  }
err:  
  /* check and return */
...
{ 
  ...
  { 
    case SOAP_SMD_DGST_SHA1:
      sha1_update((sha1_context*)data->ctx, (const void*)buf, (unsigned int)len);
      break;
...
{ unsigned int n = 0;
  ...
  { /* finalize the digest or signature computation */
    ...
    { 
      case SOAP_SMD_DGST_SHA1:
        sha1_finish((sha1_context*)data->ctx, (unsigned char*)buf);
        break;
