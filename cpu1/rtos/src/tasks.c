...
static void EYENIX_prvGenerateRunTimeStatsAndTaskInformationForTasksInList( EnxTaskInfo *pcWriteBuffer, unsigned long *ulInfoRemain, xList *pxList, signed char cStatus , unsigned long ulTotalRunTime )
{
	volatile tskTCB *pxNextTCB, *pxFirstTCB;
	unsigned long ulStatsAsPermil;
	...
	do
	{
		...
		/* Has the task run at all? || Divide by zero check. */
		if( pxNextTCB->ulRunTimeCounter == 0 || ulTotalRunTime == 0)
		{
			/* The task has used no CPU time at all. */
			pcWriteBuffer[*ulInfoRemain].ulRunTimeCounter = 0;
			pcWriteBuffer[*ulInfoRemain].ulStatsAsPermil = 0;
		}
		else
		{
			/* What percentage of the total run time as the task used?
			This will always be rounded down to the nearest integer. */
			ulStatsAsPermil = ( 1000UL * pxNextTCB->ulRunTimeCounter ) / ulTotalRunTime;

			pcWriteBuffer[*ulInfoRemain].ulRunTimeCounter = pxNextTCB->ulRunTimeCounter;
			if( ulStatsAsPermil > 0UL )
			{
				pcWriteBuffer[*ulInfoRemain].ulStatsAsPermil = ulStatsAsPermil;
			}
			else
			{
				pcWriteBuffer[*ulInfoRemain].ulStatsAsPermil = 1;
			}
		}

		strcpy((char *)pcWriteBuffer[*ulInfoRemain].pcTaskName, (char *)pxNextTCB->pcTaskName);
		pcWriteBuffer[*ulInfoRemain].cStatus = cStatus;
		pcWriteBuffer[*ulInfoRemain].uxPriority = (UINT)pxNextTCB->uxPriority;
		pcWriteBuffer[*ulInfoRemain].usStackRemaining = usStackRemaining;
		pcWriteBuffer[*ulInfoRemain].pxNextTCB = (UINT)pxNextTCB;

		(*ulInfoRemain)--;
	} while( pxNextTCB != pxFirstTCB );
}

void EYENIX_vTaskGetRunTimeCntNStats( EnxTaskInfo *etiBuffer, int nBufferSize )
{
	unsigned portBASE_TYPE uxQueue;
	unsigned long ulTotalRunTime = portGET_RUN_TIME_COUNTER_VALUE();
	unsigned long ulInfoRemain = nBufferSize - 1;
	...
	vTaskSuspendAll();
	{
		...
		do
		{
			...
			if( !listLIST_IS_EMPTY( &( pxReadyTasksLists[ uxQueue ] ) ) )
			{
				EYENIX_prvGenerateRunTimeStatsAndTaskInformationForTasksInList( etiBuffer, &ulInfoRemain, ( xList * ) &( pxReadyTasksLists[ uxQueue ] ), tskREADY_CHAR, ulTotalRunTime );
			}
		}while( uxQueue > ( unsigned short ) tskIDLE_PRIORITY );

		if( !listLIST_IS_EMPTY( pxDelayedTaskList ) )
		{
			EYENIX_prvGenerateRunTimeStatsAndTaskInformationForTasksInList( etiBuffer, &ulInfoRemain, ( xList * ) pxDelayedTaskList, tskBLOCKED_CHAR, ulTotalRunTime );
		}

		if( !listLIST_IS_EMPTY( pxOverflowDelayedTaskList ) )
		{
			EYENIX_prvGenerateRunTimeStatsAndTaskInformationForTasksInList( etiBuffer, &ulInfoRemain, ( xList * ) pxOverflowDelayedTaskList, tskBLOCKED_CHAR, ulTotalRunTime );
		}

#if ( INCLUDE_vTaskDelete == 1 )
		if( !listLIST_IS_EMPTY( &xTasksWaitingTermination ) )
		{
			EYENIX_prvGenerateRunTimeStatsAndTaskInformationForTasksInList( etiBuffer, &ulInfoRemain, ( xList * ) &xTasksWaitingTermination, tskDELETED_CHAR, ulTotalRunTime );
		}
#endif
		...
		if( !listLIST_IS_EMPTY( &xSuspendedTaskList ) )
		{
			EYENIX_prvGenerateRunTimeStatsAndTaskInformationForTasksInList( etiBuffer, &ulInfoRemain, ( xList * ) &xSuspendedTaskList, tskSUSPENDED_CHAR, ulTotalRunTime );
		}
#endif
	}
	xTaskResumeAll();
...
void EYENIX_vTaskGetTaskName(signed char * pcBuffer)
{
	portENTER_CRITICAL();
	{
		if (pxCurrentTCB == NULL) {
			...
			pcBuffer[0] = pxCurrentTCB->pcTaskName[0];
			pcBuffer[1] = pxCurrentTCB->pcTaskName[1];
			pcBuffer[2] = pxCurrentTCB->pcTaskName[2];
			pcBuffer[3] = pxCurrentTCB->pcTaskName[3];
			pcBuffer[4] = pxCurrentTCB->pcTaskName[4];
			pcBuffer[5] = pxCurrentTCB->pcTaskName[5];
			pcBuffer[6] = pxCurrentTCB->pcTaskName[6];
			pcBuffer[7] = pxCurrentTCB->pcTaskName[7];
		}
	}
	portEXIT_CRITICAL();
}
...
portBASE_TYPE xReturn;
	...
	if(pxUnblockedTCB == NULL)
	{
//		disable_htb();
		printf("pxUnblockedTCB == NULL\r\n");
		return pdFALSE;
//		BREAK;
	}
 ...
 *
 */
_NOINS static portTASK_FUNCTION( prvIdleTask, pvParameters )
{
	/* Stop warnings. */
	( void ) pvParameters;
...
_NOINS static void prvCheckTasksWaitingTermination( void )
{
	#if ( INCLUDE_vTaskDelete == 1 )
	{
...
/*-----------------------------------------------------------*/
	...
	unsigned short usStackRemaining;
		...
		do
		{
			...
			sprintf( pcStatusString, ( char * ) "%s\t\t%c\t%u\t%u\t%x\r\n", pxNextTCB->pcTaskName, cStatus, ( unsigned int ) pxNextTCB->uxPriority, usStackRemaining, (UINT)pxNextTCB );
